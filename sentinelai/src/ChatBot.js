...
const ChatBot = () => {
  ...
  // data data
  // Direct string instead of fetching document
...
Storage:�At least 190MB of free space.
  ...
  const sendMessage = async (e) => {
    ...
    try {
      ...
      console.log("Document Content:", documentContent);  // Debug: check loaded document content
      console.log("User Input:", userInput);  // Debug: check the user input

      // Make a request to LM Studio's completion endpoint with the document content
      const response = await fetch("http://localhost:1234/v1/chat/completions", { // Update the endpoint if required
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          model: "llama-2-7b-chat", // Specify the model you're using
          messages: [
            { role: "system", content: `You are given the following document. Answer the question strictly based on its content. Document: "${documentContent}"` },
      ...
      console.log("Response:", data.choices[0]?.text);  // Check the response data

      const botMessage = data.choices && data.choices[0]?.text.trim();
      ...
      if (botMessage) {
        ...
        setMessages((prevMessages) => [
          ...prevMessages,
          { sender: "bot", text: "Sorry, I could not find an answer to your question." }
        ]);
      }
    } catch (error) {
